...
// Aksen neon berbeda tiap hari di atas background gelap (dipakai layout card)
...
// Tema warna untuk mode compact — identitas "Aurora Dark"
const AURORA_COMPACT_THEME: CompactTheme = {
  background: "bg-gradient-to-b from-[#0a0a0f] via-[#12121c] to-[#0a0a0f]",
  titleColor: "bg-gradient-to-r from-white via-fuchsia-200 to-cyan-200 bg-clip-text text-transparent",
  subtitleColor: "text-white/40",
  dayLabelColor: "text-white/60",
  dividerColor: "bg-white/15",
  timeColor: "text-cyan-300",
  courseColor: "text-white",
  courseSubColor: "text-white/40",
};

const AuroraDarkTemplate = forwardRef<HTMLDivElement, Props>(
  ({ data, width, height, sizeId }, ref) => {
      ...
      return (
        ...
        >
          <div style={{ width: COMPACT_DESIGN_WIDTH, height: COMPACT_DESIGN_HEIGHT }}>
            <CompactScheduleLayout data={data} theme={AURORA_COMPACT_THEME} />
          </div>
        </ScaledWallpaperCanvas>
      );
    }
    ...
    return (
      <ScaledWallpaperCanvas ref={ref} width={width} height={height} background="#0a0a0f">
        <div
