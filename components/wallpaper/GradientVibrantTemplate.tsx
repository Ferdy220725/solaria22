...
// Tema warna untuk mode compact — identitas "Gradient Vibrant"
const GRADIENT_VIBRANT_COMPACT_THEME: CompactTheme = {
  background: "bg-gradient-to-br from-purple-700 via-fuchsia-600 to-orange-500",
  titleColor: "text-white drop-shadow-lg",
  subtitleColor: "text-white/70",
  dayLabelColor: "text-white/90",
  dividerColor: "bg-white/25",
  timeColor: "text-white",
  courseColor: "text-white",
  courseSubColor: "text-white/60",
};

const GradientVibrantTemplate = forwardRef<HTMLDivElement, Props>(
  ({ data, width, height, sizeId }, ref) => {
      ...
      return (
        ...
        >
          <div style={{ width: COMPACT_DESIGN_WIDTH, height: COMPACT_DESIGN_HEIGHT }}>
            <CompactScheduleLayout data={data} theme={GRADIENT_VIBRANT_COMPACT_THEME} />
          </div>
        </ScaledWallpaperCanvas>
      );
    }
    ...
    return (
      <ScaledWallpaperCanvas ref={ref} width={width} height={height} background="#7e22ce">
        <div
